...
class _ProfileState extends State<ProfileScreen> {
  ...
  Future<void> _logout() async { await _s.deleteAll(); if (mounted) context.go('/'); }
  ...
  void _showAbout() {
    showModalBottomSheet(context: context, backgroundColor: Colors.transparent, builder: (_) => Container(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        RichText(text: TextSpan(style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w700, fontFamily: 'DM Sans'), children: [
          TextSpan(text: 'Chap', style: TextStyle(color: C.t1)),
          const TextSpan(text: 'smart', style: TextStyle(color: C.btc)),
        ])),
        const SizedBox(height: 4),
        Text('Version 2.0.0', style: TextStyle(fontSize: 13, color: C.t3)),
        const SizedBox(height: 16),
        Text('Bitcoin \u2194 Mobile Money for Tanzania', style: TextStyle(fontSize: 14, color: C.t2)),
        const SizedBox(height: 6),
        Text('Moshi, Kilimanjaro', style: TextStyle(fontSize: 12, color: C.t3)),
        const SizedBox(height: 20),
        Row(mainAxisAlignment: MainAxisAlignment.center, children: [
          Icon(Icons.email_rounded, color: C.t3, size: 14), const SizedBox(width: 6),
          Text('support@chapsmart.com', style: TextStyle(fontSize: 13, color: C.t2)),
        ]),
        const SizedBox(height: 8),
        Row(mainAxisAlignment: MainAxisAlignment.center, children: [
          Icon(Icons.language_rounded, color: C.t3, size: 14), const SizedBox(width: 6),
          Text('www.chapsmart.com', style: TextStyle(fontSize: 13, color: C.t2)),
        ]),
        const SizedBox(height: 24),
        BtnSecondary(label: 'Close', onTap: () => Navigator.pop(context)),
      ]),
    ));
  }
  ...
  Future<void> _deleteAccount() async {
      ...
      builder: (ctx) => Container(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          Container(width: 56, height: 56,
            ...
            child: const Icon(Icons.warning_rounded, color: C.red, size: 28)),
          const SizedBox(height: 16),
          Text('Delete Account?', style: TextStyle(fontSize: 20, fontWeight: FontWeight.w700, color: C.t1)),
          const SizedBox(height: 8),
          Text(
            'Hii ni hatua ya kudumu. Akaunti yako, historia yote ya muamala, na data yote itafutwa. Huwezi kuirejesha.',
            style: TextStyle(fontSize: 13, color: C.t2, height: 1.6),
            textAlign: TextAlign.center,
          ),
          const SizedBox(height: 20),
          Btn(label: 'Futa Akaunti', onTap: () => Navigator.pop(ctx, true), icon: Icons.delete_forever_rounded),
          const SizedBox(height: 10),
          BtnSecondary(label: 'Ghairi', onTap: () => Navigator.pop(ctx, false)),
        ]),
      ),
    );
    ...
    try {
      ...
      await _s.deleteAll();
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Akaunti imefutwa'), backgroundColor: C.green));
        context.go('/');
      }
    } catch (e) {
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Imeshindwa kufuta: \$e'), backgroundColor: C.red));
    }
  }
      ...
      body: SafeArea(child: SingleChildScrollView(padding: const EdgeInsets.all(22), child: Column(children: [
        ...
        Container(
          ...
          decoration: BoxDecoration(
            ...
            borderRadius: BorderRadius.circular(18), border: Border.all(color: C.btc.withOpacity(0.15)),
          ...
          child: Column(children: [
            ...
            const SizedBox(height: 4),
            GestureDetector(
              onTap: () { Clipboard.setData(ClipboardData(text: _acc ?? '')); ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Copied'), backgroundColor: C.t1, behavior: SnackBarBehavior.floating)); },
            ...
            if (_nostr != null) ...[
              ...
              Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
                decoration: BoxDecoration(color: C.purple.withOpacity(0.08), borderRadius: BorderRadius.circular(20)),
                child: Row(mainAxisSize: MainAxisSize.min, children: [
                  const Icon(Icons.key_rounded, color: C.purple, size: 12), const SizedBox(width: 4),
                  Text('${_nostr!.substring(0, 8)}...', style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w500, color: C.purple)),
                ])),
            ],
          ]),
        ),
        ...
        // Nostr link
        if (_nostr == null) ...[
          GestureDetector(onTap: _linkNostr, child: Container(
            ...
            child: Row(children: [
              Container(padding: const EdgeInsets.all(10), decoration: BoxDecoration(color: C.purple.withOpacity(0.08), borderRadius: BorderRadius.circular(10)),
                child: const Icon(Icons.link_rounded, color: C.purple, size: 18)),
              const SizedBox(width: 12),
              const Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text('Connect Nostr', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: C.t1)),
                Text('Link your Nostr key for easy login', style: TextStyle(fontSize: 12, color: C.t3)),
              ])),
        ...
        // Stats
        Row(children: [
          Expanded(child: StatCard(label: 'FEE RATE', value: Fmt.pct(fee), valueColor: C.green)),
          const SizedBox(width: 10),
          Expanded(child: StatCard(label: 'TRANSACTIONS', value: '$txCount', valueColor: C.blue)),
        ]),
        const SizedBox(height: 10),
        StatCard(label: 'TOTAL SPEND', value: Fmt.tzs(cumul), valueColor: C.btc),
        const SizedBox(height: 20),

        // Tier progress
        const SecHead(title: 'Tier Progress'),
        const SizedBox(height: 12),
        Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: C.card, borderRadius: BorderRadius.circular(14), border: Border.all(color: C.border)),
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              TierBadge(tier: tier),
              if (nextTier != null) Text('Next: $nextTier', style: const TextStyle(fontSize: 12, color: C.t3)),
            ]),
            const SizedBox(height: 12),
            ...
            const SizedBox(height: 8),
            Text(tier == 'GOLD' ? 'Maximum tier!' : '${(progress * 100).toStringAsFixed(0)}% to ${nextTier ?? "next"}', style: const TextStyle(fontSize: 11, color: C.t3)),
          ])),
        const SizedBox(height: 20),

        // Fee tiers
        const SecHead(title: 'Fee Tiers'),
        const SizedBox(height: 12),
        ...[ ('BRONZE', 2.2, 'Entry level'), ('SILVER', 1.87, 'Regular sender'), ('GOLD', 1.32, 'Power user') ].map((t) => Container(
          ...
          child: Row(children: [
            TierBadge(tier: t.$1), const SizedBox(width: 12),
            Expanded(child: Text(t.$3, style: const TextStyle(fontSize: 13, color: C.t2))),
        ...
        GestureDetector(
          ...
          child: Container(
            ...
            child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
              Icon(Icons.logout_rounded, color: C.t2, size: 18), const SizedBox(width: 8),
              Text('Sign Out', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w600, color: C.t2)),
            ]),
          ),
        ),
        const SizedBox(height: 10),
        ...
        GestureDetector(
          ...
          child: Container(
            ...
            child: const Row(mainAxisAlignment: MainAxisAlignment.center, children: [
              Icon(Icons.delete_forever_rounded, color: C.red, size: 18), SizedBox(width: 8),
              Text('Delete Account', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w600, color: C.red)),
            ]),
          ),
        ),
      ]))),
...
class _SettingsTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return GestureDetector(onTap: onTap, child: Container(
      ...
      child: Row(children: [
        Container(width: 38, height: 38, decoration: BoxDecoration(color: color.withOpacity(0.08), borderRadius: BorderRadius.circular(10)),
          child: Icon(icon, color: color, size: 18)),
        const SizedBox(width: 12),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(title, style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: C.t1)),
          Text(subtitle, style: TextStyle(fontSize: 12, color: C.t3)),
        ])),
